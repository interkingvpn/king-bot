...
say(`Desarrollado por • INTER•KING`, {
    font: 'console',
    align: 'center',
    colors: ['blueBright']
})
...
if (!methodCodeQR && !methodCode && !fs.existsSync(`./${sessions}/creds.json`)) {
    do {
        option = await question(colorOption('⌨ Seleccione una opción:\n') + qrOption('1. Con código QR\n') + textOption('2. Con código de texto de 8 dígitos\n--> '))

        if (!/^[1-2]$/.test(option)) {
            console.log(chalk.bold.redBright(`✦ Solo se permiten números 1 o 2, sin letras ni símbolos especiales.`))
        }
    ...
    if (option === '2' || methodCode) {
        ...
        if (!conn.authState.creds.registered) {
            ...
            if (!!phoneNumber) {
                ...
                do {
                    phoneNumber = await question(chalk.bgBlack(chalk.bold.greenBright(`✦ Por favor ingrese el número de WhatsApp.\n${chalk.bold.yellowBright(`✏  Ejemplo: 255××××××`)}\n${chalk.bold.magentaBright('---> ')}`)))
...
async function connectionUpdate(update) {
    ...
    if (update.qr != 0 && update.qr != undefined || methodCodeQR) {
        if (option == '1' || methodCodeQR) {
            console.log(chalk.bold.yellow(`\n❐ ESCANEE EL CÓDIGO QR, EXPIRA EN 45 SEGUNDOS`))
        }
    }
    if (connection == 'open') {
        console.log(chalk.bold.green('\n❀ Lazack Device conectado con éxito ❀'))
    }
    let reason = new Boom(lastDisconnect?.error)?.output?.statusCode
    if (connection === 'close') {
        if (reason === DisconnectReason.badSession) {
            console.log(chalk.bold.cyanBright(`\n⚠︎ SIN CONEXIÓN, ELIMINE LA CARPETA ${global.sessions} Y ESCANEE EL CÓDIGO QR ⚠︎`))
        } else if (reason === DisconnectReason.connectionClosed) {
            console.log(chalk.bold.magentaBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ☹\n┆ ⚠︎ CONEXIÓN CERRADA, RECONECTANDO....\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ☹`))
            await global.reloadHandler(true).catch(console.error)
        } else if (reason === DisconnectReason.connectionLost) {
            console.log(chalk.bold.blueBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ☂\n┆ ⚠︎ CONEXIÓN PERDIDA CON EL SERVIDOR, RECONEXIÓN EN PROCESO....\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ☂`))
            await global.reloadHandler(true).catch(console.error)
        } else if (reason === DisconnectReason.connectionReplaced) {
            console.log(chalk.bold.yellowBright(`\n╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗\n┆ ⚠︎ CONEXIÓN REEMPLAZADA, OTRA SESIÓN HA SIDO INICIADA, POR FAVOR CIERRE LA SESIÓN ACTUAL PRIMERO.\n╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄ • • • ┄┄┄┄┄┄┄┄┄┄┄┄┄┄ ✗`))
...
setInterval(async () => {
    ...
    await clearTmp()
    console.log(chalk.bold.cyanBright(`\n╭» ❍ MULTIMEDIA ❍\n│→ ARCHIVOS EN LA CARPETA TMP ELIMINADOS\n╰― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ⌫ ♻`))
}, 1000 * 60 * 4) // 4 min 
...
http.createServer((req, res) => {
  ...
  res.end('KING•BOT V2 está en funcionamiento\n')
}).listen(PORT, () => {
  console.log(chalk.bold.green(`Servidor en ejecución en http://localhost:${PORT}/`))
})