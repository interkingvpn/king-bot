...
export async function sanJadiBot(options, text) {
  ...
  try {
    ...
    conn.reply(m.chat, `🕸 *Usa el comando correctamente »* ${usedPrefix + command} code`, m);
    return;
  }
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    // ⚡ Maneja las actualizaciones de conexión (QR, emparejamiento, desconexión, etc.)
    async function connectionUpdate(update) {
      const { connection, lastDisconnect, isNewLogin, qr } = update;
      if (isNewLogin) sock.isInit = false;

      // 🔲 Mostrar QR si no se usa modo código
      if (qr && !mcode) {
        if (m?.chat) {
      ...
      // 🔁 Control de reconexión y errores
      if (connection === "close") {
        if (
          reason === 428 ||
          ...
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`Intentando reconectar con +${path.basename(pathSanJadiBot)}...`)}`);
          await sleep(1000);
        ...
        if (reason === 440) {
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`La conexión para +${path.basename(pathSanJadiBot)} fue reemplazada por otra sesión activa.`)}`);
        }

        if (reason == 405 || reason == 401) {
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`No se encontró ninguna sesión activa para +${path.basename(pathSanJadiBot)}.`)}`);
          fs.rmdirSync(pathSanJadiBot, { recursive: true });
        }

        if (reason === 500) {
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`Sesión perdida para +${path.basename(pathSanJadiBot)}, eliminando datos..`)}`);
          return creloadHandler(true).catch(console.error);
        }

        if (reason === 515) {
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`Reinicio automático para +${path.basename(pathSanJadiBot)}.`)}`);
          await creloadHandler(true).catch(console.error);
        }

        if (reason === 403) {
          console.log(`\n${chalk.bold.whiteBright.bgRed("ADVERTENCIA:")} ${chalk.bold.magentaBright(`Sesión cerrada para +${path.basename(pathSanJadiBot)}.`)}`);
          fs.rmdirSync(pathSanJadiBot, { recursive: true });
        }
      }
      ...
      if (connection == "open") {
        ...
        if (m && conn && isCode && commandFlags[m.sender]) {
          await conn.sendMessage(m.chat, { text: `🕸 Nuevo *Sub Bot* vinculado correctamente.` }, { quoted: m });
...
// 📰 Unirse automáticamente a canales del bot
