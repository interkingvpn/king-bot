...
if (opcion === '2' && !fs.existsSync(`./${authFolder}/creds.json`)) {
    ...
    if (phoneNumber) {
        ...
        if (!numeroTelefono.match(/^\d+$/) || !Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
            ...
            console.log(chalk.yellow('[ ℹ️ ] Formato correcto: +5493765142705'));
            process.exit(1);
        }
    } else {
        while (true) {
            ...
            if (numeroTelefono.match(/^\d+$/) && Object.keys(PHONENUMBER_MCC).some(v => numeroTelefono.startsWith(v))) {
                ...
                console.log(chalk.red('[ ● ] Número inválido. Use formato: +5493765142705'));
            }
        }
    }
...
setInterval(async () => {
  ...
  const bio = `• Activo: ${uptime} | KING•BOT`;
